...
module tb_dc3_pack;
  ...
  reg        inj_arm  = 0;
  reg        inj_done = 0;
  wire [7:0] a_link_data = a_link_data_raw ^ (inj_arm & ~inj_done ? inj_mask : 8'h00);
  always @(negedge a_link_clk) if (inj_arm && !inj_done) inj_done <= 1'b1;
  ...
  always @(posedge clk_b) begin
    ...
    if (pm_valid && pm_ready && capturing) begin
      stage[sidx] <= pm_data;
      sidx <= sidx + 1;
    end
    if (pm_eof) begin
  ...
  task automatic wait_for_ok(input integer n);
    ...
    begin
      guard = 0;
      while ((cnt_ok < n) && (guard < 300_000)) begin
        @(posedge clk_b);
        guard = guard + 1;
      end
      if (cnt_ok < n) begin
        errors = errors + 1;
        $display("TIMEOUT waiting for %0d packets, have %0d", n, cnt_ok);
  ...
  initial begin
    ...
    inj_mask = 8'h01; inj_done = 0; inj_arm = 1;
